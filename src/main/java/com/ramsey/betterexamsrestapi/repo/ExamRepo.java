...
public interface ExamRepo extends CrudRepository<Exam, Long> {
	...
	@Query("SELECT e FROM Teacher t, IN(t.students) s, IN(t.exams) e WHERE s.user.username = :username AND e.name LIKE %:examName%")
	List<Exam> searchExamForStudent(String username, String examName, Pageable pageable);
	@Query("SELECT e FROM Teacher t, IN(t.exams) e WHERE t.user.username = :username AND e.name LIKE %:examName%")
